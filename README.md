...
**Known gap:** agent/skill content currently references `templates/...` by a path relative to this repo's root (e.g. `templates/paper-page-template.md`). This still doesn't resolve once installed and invoked from an unrelated project. `${CLAUDE_PLUGIN_ROOT}` looked like the fix Claude Code's plugin system provides for exactly this, but it's scoped to hook/monitor/MCP *command* config fields — not to skill/agent Markdown prose, and not exposed as a Bash-tool environment variable (verified directly: `echo $CLAUDE_PLUGIN_ROOT` under `--plugin-dir` returns empty). Still open; see [PROJECT_CONTEXT.md](PROJECT_CONTEXT.md).
