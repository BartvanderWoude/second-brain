...
Once a vault exists, a **topic deep-dive** (`second-brain-topic-deep-dive`) gives one of its topic notes a literature search of its own: it deepens the note, or creates it if it does not exist, from the new papers, and links it into the vault. It reuses stages 1–5 with a short drafted Q&A.
...
### Citation chasing needs no key

The citation chaser reads [OpenAlex](https://openalex.org) through `scripts/find_papers.py`. OpenAlex needs no key: without one, its daily budget covers about 1,000 filter requests, and a chase round uses around ten. For large vaults or several runs a day, set `OPENALEX_API_KEY` in `.env`; a free key raises the budget tenfold. OpenAlex is used rather than Semantic Scholar because Semantic Scholar returns empty reference lists for many clinical publishers: 0 references for two core reRD papers where OpenAlex has 35 and 29.
...
│   ├── second-brain-citation-chaser.md            # stage 3, after the legs: exhaustive coverage of the core question
...
│   ├── research-problem-intake/deep-dive.md       # its topic deep-dive branch
│   ├── second-brain-pipeline/SKILL.md              # orchestrator: runs stages 1–5 end to end, holds the stage-4 checkpoint
│   ├── second-brain-pipeline/deep-dive.md          # its topic deep-dive branch
│   └── second-brain-topic-deep-dive/SKILL.md       # entry point: a literature search for one topic note of an existing vault
├── scripts/
│   ├── fetch_fulltext.py                          # stage 3: upgrades a saved paper record to full text (open access)
│   ├── stage_prep.py                              # stages 3–5: duplicate merge, fan-out plan, keyword index, topic digests, deep-dive topic reports
│   ├── find_papers.py                             # stage 3: whole PubMed hit sets, OpenAlex citation chasing, saving chosen papers
...
- **`second-brain-citation-chaser` agent** — stage 3, after the discovery legs. Covers the profile's core question exhaustively, independently of how the legs worded their queries: the core question is marked `core_questions` on a topic profile, and is the direct comparators on a problem profile. It runs the profile's `recall_probes` as whole PubMed hit sets, then chases citations one hop both ways from the vault's core papers through OpenAlex, round after round until a round adds nothing. It screens titles first and reads abstracts only where a title leaves it unsure. It exists because one run found 5 of the 19 papers on its core question and reported a clean run. Chasing from the 10 core papers it did find recovered 10 of the 14 it missed, including four classical models no query had matched.
- **`scripts/find_papers.py`** — numbered candidate lists for the agents that screen papers, and saving the chosen ones. `pubmed` retrieves every query's whole hit set and reports its count. A query too large to fetch whole is flagged, never cut to its top hits without saying so. `chase` does the OpenAlex hop. `show` prints abstracts on demand, and `add` saves chosen candidates as identity-header records from PubMed's, arXiv's or OpenAlex's metadata, then fetches their open-access full text. The biomedical leg and the citation chaser both search and save through it.
...
- **`second-brain-topic-deep-dive` skill** — a literature search for one topic note of a vault that already exists: it deepens the note when it exists and creates it when it does not. It resolves the note with `stage_prep.py topic`, which also warns when the vault uses old-style ids, then runs a short drafted Q&A through `research-problem-intake` and stages 3–5 through `second-brain-pipeline` on a deep-dive profile: a topic profile scoped to that one topic. New papers go into the vault's own paper vault and are summarized against the vault's profile, so they count in its index like any other paper. The note is written deeper than a normal topic note and marked `deep_dive`, so a later re-run cannot shrink it. Vault-build now gives every topic note a `## Related topics` section, the other notes sharing at least two of its papers, which is how the new note links to the rest of the vault's topics.
